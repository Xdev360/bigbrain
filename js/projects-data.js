...
window.BB_PROJECTS = [
  ...
  },
  {
    ...
    name: 'Nicovellor',
    year: '2025',
    tagline: 'Ultra-wealthy network brand — clothing and art for Velourians who buy access, not noise.',
    about: 'Private luxury brand for Velourians — clothing and art for ultra-high-net-worth buyers who buy access, not noise.',
    solution: 'Coded black-and-white luxury: buy clothing, buy art, feel the network — never a normal storefront.',
    liveUrl: '',
    tags: ['Brand', 'Web', 'Graphic', 'App'],
    industry: 'Luxury / fashion / art',
    productType: 'Brand + web + product',
    stage: 'Shipped',
    ...
    problem: 'Nicovellor sells more than clothes — it sells network access for ultra-high-net-worth buyers (Velourians). The brand needed coded black-and-white luxury: buy clothing, buy art, never feel like a normal storefront.',
    phases: [
      {
        id: 'brand-identity',
        label: 'Brand identity',
    ...
    outcome: {
      title: 'What it moved',
      body: 'A brand and product that feel like a private network — clothing and art under one coded system for Velourians.',
      metrics: [
        { value: 'UHNW', label: 'Positioned for ultra-high-net-worth buyers' },
...
window.BB_SERVICES = [
  {
    id: 'brand-identity',
    label: 'Brand identity',
    title: 'Marks and systems people remember',
    body: 'Logos, colour, type, and brand kits — built so every touchpoint feels like one company.',
    guide: 'Brand identity — marks and systems that stick.',
    pitch: 'A strong identity is the cheapest long-term ad you will ever run.'
  },
  {
    id: 'web-design',
    label: 'Web design',
    title: 'Sites that explain and convert',
    body: 'Marketing sites and landings with a clear story and a clean path to action — Framer or code.',
    guide: 'Web design — story first, then the click.',
    pitch: 'Your site should sell while you sleep.'
  },
  {
    id: 'app-design',
    label: 'App design',
    title: 'Product UI people finish',
    body: 'Mobile and product interfaces shaped around real tasks — fewer drop-offs, clearer next steps.',
    guide: 'App design — interfaces people actually finish.',
    pitch: 'Good UX is fewer people giving up halfway.'
  },
  {
    id: 'graphic-design',
    label: 'Graphic design',
...
window.BB_getProject = function(id){
  var list = window.BB_PROJECTS || [];
  for(var i=0;i<list.length;i++){ if(list[i].id===id) return list[i]; }
  return null;
};

window.BB_getService = function(id){
  var list = window.BB_SERVICES || [];
  for(var i=0;i<list.length;i++){ if(list[i].id===id) return list[i]; }
  return null;
};

window.BB_projectsForService = function(serviceId){
  return (window.BB_PROJECTS||[]).map(function(p){
    var phase = null;
    (p.phases||[]).forEach(function(ph){ if(ph.id===serviceId) phase = ph; });
    if(!phase) return null;
    return { project:p, phase:phase };
  }).filter(Boolean);
};
